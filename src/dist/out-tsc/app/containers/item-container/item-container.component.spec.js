...
import { MdButtonModule, MdCardModule, MdCheckboxModule, MdChipsModule, MdGridListModule, MdIconModule, MdInputModule, MdListModule, MdSelectModule, MdSidenavModule, MdToolbarModule } from "@angular/material";
...
//
// }
var setMockAreaRoute = function (route, mock) {
    route.params = Observable.of({ id: mock, areaId: 1 });
    spyOn(route.params, 'subscribe').and.callThrough();
};
var setMockRoute = function (route) {
    route.params = Observable.of({});
    spyOn(route.params, 'subscribe').and.callThrough();
};
describe('ItemContainerComponent', function () {
    var component;
    var fixture;
    var route;
    var store;
    ...
    it('should dispatch request for item data', fakeAsync(function () {
        setMockAreaRoute(route, '1');
        component.ngOnInit();
        tick();
        expect(store.dispatch).toHaveBeenCalledWith(new fromItem.ItemRequest('1'));
    }));
    it('should not dispatch request if id parameter is not supplied.', fakeAsync(function () {
        setMockRoute(route);
        component.ngOnInit();
        tick();
        expect(component.id).toBeUndefined();
    }));
    it('should dispatch request for related items.', fakeAsync(function () {
        setMockAreaRoute(route, '1');
        component.id = '1';
    ...
    it('should clear related items on init', fakeAsync(function () {
        setMockAreaRoute(route, '1');
        component.ngOnInit();
        tick();
        expect(store.dispatch).toHaveBeenCalledWith(new fromRelated.ClearRelatedItems());
    }));
    it('should reset the item reducer', fakeAsync(function () {
        setMockAreaRoute(route, '1');
        component.ngOnInit();
        tick();
...
//# sourceMappingURL=/Users/mspalti/willamette/commons/commons-6.28.17/src/client/app/containers/item-container/item-container.component.spec.js.map